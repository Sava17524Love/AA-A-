...
class ImageProcessor():
    ...
    def do_bw(self):
        ...
        self.saveImage()
        image_path = os.path.join(self.dir, self.seve_dir, self.filename)
        self.showImage(image_path)

    def saveImage(self):
        path = os.path.join(self.dir, self.seve_dir)
        if not (os.path.exists(path) or os.path.isdir(path)):
            os.mkdir(path)
        image_path = os.path.join(path, self.filename)
        self.image.seve(image_path)
...
def showChosenImage():
    if lw_files.currentRow() >= 0:
        filename = lw_files.currentItem().test()
